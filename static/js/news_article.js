...
const mode = (window.location.href).split('/news/')[1].split('/')[0]


const unit = (window.location.href).split('/news/')[1].split('/')[1]
const team = (window.location.href).split('/news/')[1].split('/')[2]
...
for (var n = 1; n < r + 1; n++) {
    markerOBJ[n] = 1
    audioOBJ[n] = null
    uploadOBJ[n] = ['upbtn1', 'uplink1']
}
console.log('MARKER', markerOBJ)
console.log('audio', audioOBJ)
console.log('uploadBTN', uploadOBJ)
...
function startVue(){
  ...
  let vue = new Vue({
    ...
    methods: {
        ...
        storeData : function(key, mode) {
            //reset the testOBJ
            this.testOBJ = testOBJ

            if (mode == 'news') {
                this.ansOBJ[key]['user'] = document.getElementById(key +  'u').innerHTML
                this.ansOBJ[key]['text'] = document.getElementById(key + 't').value
            } else {
                this.ansOBJ[key]['user'] = document.getElementById(key +  'u').innerHTML
